...
#         color: #ff9f43;
#     }
#     .search-section, .details-section, .recommend-section {
#         margin-top: 2rem;
#     }
...
# # TMDB API key (use environment variables or secrets for security)
# TMDB_API_KEY = "6bc3065293c944b5ad11cb7cd15c076e"


# def fetch_movie_details(movie_id):
#     """Fetch detailed information for a given movie ID using TMDB API."""
#     response = requests.get(
...
#     """Fetch the poster path for a given movie ID."""
#     data = fetch_movie_details(movie_id)
#     return (
#         f"https://image.tmdb.org/t/p/w780/{data['poster_path']}"
...
#         with cols[1]:  # Right column for recommendations
#             st.markdown("<div class='recommend-section'>", unsafe_allow_html=True)
#             st.subheader(f"🔍 Recommendations based on **{selected_movie}**")
#             try:
#                 movies, posters = preprocess.recommend(new_df, selected_movie, r"Files/similarity_tags_tags.pkl")
...
#                                 if st.button(movies[i + j], key=f"movie_{i + j}"):
#                                     st.session_state["selected_movie_name"] = movies[i + j]
#                                     st.rerun()  # Update to show selected movie details
#             except Exception as e:
#                 st.error(f"Error fetching recommendations: {e}")
#             st.markdown("</div>", unsafe_allow_html=True)
...
# def fetch_popular_movies(new_df, num_movies=16):
#     """Fetch popular movies manually or randomly from the dataset."""
#     # Fallback: Randomly select popular movies if no suitable metric is available
...
#     # App Title
#     st.markdown("<h1 class='title'>🎥 Movie Recommender</h1>", unsafe_allow_html=True)
#     st.markdown("<h2 class='title'>🎞️Over 4000 movies and its recommendations </h2>", unsafe_allow_html=True)
#     st.markdown("<h2 class='title'>       🍿📽️📺🎧 </h2>", unsafe_allow_html=True)
...
#         st.markdown("<h3> Dont forget to click SEARCH")
#         if search_query:
#             movie_found = new_df[new_df["title"].str.lower() == search_query.lower()]
#             if not movie_found.empty:
#                 st.session_state["selected_movie_name"] = movie_found["title"].iloc[0]
...
#     # Display Popular Movies Section (only if no movie is selected)
#     if not st.session_state["search_performed"]:
#         st.markdown("<div class='recommend-section'>", unsafe_allow_html=True)
#         st.subheader("🔥 Popular Movies")
#         try:
#             popular_titles, popular_posters = fetch_popular_movies(new_df)
#             for i in range(0, len(popular_titles), 4):
#                 cols = st.columns(4)
#                 for j, col in enumerate(cols):
#                     if i + j < len(popular_titles):
#                         with col:
#                             st.image(popular_posters[i + j], width=200)  # Set poster width to 200
...
# def fetch_top_rated_movies(new_df, num_movies=16):
...
#     # App Title
#     st.markdown("<h1 class='title'>🎥 Movie Recommender</h1>", unsafe_allow_html=True)
#     st.markdown("<h2 class='title'>🎞️ Over 4000 movies and its recommendations 🍿📽️📺🎧</h2>", unsafe_allow_html=True)
...
#                 for i in range(0, len(top_rated_titles), 4):
#                     row_cols = st.columns(4)
#                     for j, col in enumerate(row_cols):
...
#     layout="wide",  # Set layout to wide for full-page width
#     page_icon="🎥",
#     initial_sidebar_state="collapsed",  # Collapsed sidebar removes fullscreen button
# )
...
#     }
#     iframe {
#         display: none; /* Hide the fullscreen button from Streamlit components */
#     }
#     .separator {
#         border-left: 2px solid #ff9f43;
#         height: 100%;
...
#     background-color:rgb(181,200,239);
#     }
#     .st-emotion-cache-18ni7ap{
#     background-color:rgb(138,179,218);
#     }
#     .st-emotion-cache-1qr7fo3.e1f1d6gn2{
#     background-color:rgb(181,200,239);
#     }
#     .main-container {
#         padding: 2rem;
...
# # Regular search bar, not a dropdown
...
#             movie_found = new_df[new_df["title"].str.lower() == search_query.strip().lower()]
#         if not movie_found.empty:
#             st.session_state["selected_movie_name"] = movie_found["title"].iloc[0]
#             st.session_state["search_performed"] = True
...
#         padding: 2rem;
#         border-radius: 8px;
#         background-color: rgba(40, 44, 52, 0.85); /* Slight transparency for better readability */
#     }
#     .title {
#         text-align: center;
#         font-weight: 600;
...
#     }
#     iframe, a {
#         display: none !important; /* Hide iframe and links */
#     }
#     button[title="View fullscreen"] {
...
#     top_rated_movies = new_df.sample(n=min(num_movies, len(new_df)), random_state=43)
#     titles = top_rated_movies["title"].tolist()
#     movie_ids = top_rated_movies["movie_id"].tolist()
#     posters = [preprocess.fetch_posters(movie_id) for movie_id in movie_ids]
#     return titles, posters
...
#                             with col:
#                                 st.image(posters[i + j], width=150)
#                                 if st.button(movies[i + j], key=f"rec_{i + j}"):
#                                     st.session_state["selected_movie_name"] = movies[i + j]
#                                     st.session_state["search_performed"] = True
#                                     st.rerun()
#             except Exception as e:
...
st.markdown(
    ...
    .st-emotion-cache-1avcm0n{
        background: rgb(92 123 182);
    }
    .st-emotion-cache-13nj992 e11y4ecf0{
        color: white;
        background: white;
    }
    .base-input{
